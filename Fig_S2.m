...
%
% Content:
% - This is the extended version of Fig2_GH
% - Instead of plotting only fish 1, we also plot the other 4 fish in a
% loop.
...
%% [USER INPUT] To plot the supplemental figure for a specific fish (range
% 1-5), select the appropriate index
target_fish_idx = 3; % this can be an index 1, 2, 3, 4, or 5. Please change it for plotting purpose.
...
for i = 1 : 5 % Supplemental figures have all the fish
...
for i = target_fish_idx
    ...
    for k = 1:length(ytail_fft_pos_comb)
        ...
        if size(ytail_fft_pos_comb{k},1)>3 % Toggles between "all trials" vs. "good trials only" uncomment for supplementary figure
            ...
            xlabel('Freq (Hz)')
            ylabel('DFT Position')
            title(['Fish ', num2str(i), ' Supp Fig 2'],'FontWeight','normal','FontSize',16)
            cbh = colorbar;
            ...
            saveas(gcf, [out_path, '\suppl_fig02_fish_0', num2str(i), '_fft_tail_amp.png';]);
            saveas(gcf, [pdf_path, '\suppl_fig02_fish_0', num2str(i), '_fft_tail_amp.pdf';]);
            ...
            saveas(gcf, [out_path, '\suppl_fig02_fish_0', num2str(i), '_fft_tail_vel.png';]);
            saveas(gcf, [pdf_path, '\suppl_fig02_fish_0', num2str(i), '_fft_tail_vel.pdf';]);
