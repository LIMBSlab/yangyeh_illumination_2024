...
for field_to_plot = 'X'
    ...
    for i = 1 : num_fish
        ...
        data_smoothed_centered{i} = smooth(data - 1*(mean_value_this - 1*mean_value_all),3) * 1e4;
        data_unsmoothed_centered{i} = (data - (mean_value_this - 1*mean_value_all)) * 1e4;

 
            all_lux = [all_lux, lux{i}];
            all_data_pts_processed_1 = [all_data_pts_processed_1; data_smoothed_centered{i}];
            all_data_pts_processed_2 = [all_data_pts_processed_2; data_unsmoothed_centered{i}];
    ...
    [fitresult2, gof2] = createSigmoidFit(field_name, X2, Y2);
    ...
    for i = 1 : num_fish
        h (2+i) = scatter((lux{i}), (data_smoothed_centered{i}),80,colorMap(i, :),'filled','MarkerFaceAlpha',0.9);
    end
    ...
    legend(h,{'sigmoidal fit','95% PI','fish 1','fish 2','fish 3','fish 4','fish 5'},'Location','northeast','edgecolor','none','fontsize',16)
