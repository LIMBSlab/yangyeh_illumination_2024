...
% Updated 11.08.2024
% LIMBS Lab
% Author: Debojyoti Biswas, Huanying (Joy) Yeh

% Experiment Name: Eigenmannia Virescens Luminance + Locomotion Comparisons
%
% Content:
% - Calculate the Tail FFT results and save it to the "res.mat" file in
% "data_structures"
% - Fig2_GH: Plot the tail FFT amplitudes and velocities of a sample fish
% 
% Output (for archive):
% "fig02g01_body_bending_vs_illuminance.png", with video frames overlaying the rainbow colors.
% "fig02g02_body_bending_timeline.png", it's the right panel with rainbow
% color timelines
% "fig02g03" and "fig02g04", same panels but for a different trial.
...
t = (0:L-1)*1/Fs; % Time vector 1 x 500
...
for i = 1
    ...
    for il = 1: num_ils
        ...
        % Populate lux values first
        for trial_idx = 1 : num_trials
            raw(i).luminances(il).lux = all_fish(i).lux_values(il);
            res(i).luminances(il).lux = all_fish(i).lux_values(il);
        end

        % These are for individual trials
        raw(i).luminances(il).x_tail = {};
        raw(i).luminances(il).y_tail = {};
        ...
        res(i).luminances(il).y_cwt_amp = {};
        res(i).luminances(il).y_cwt_vel = {};

        for trial_idx = 1 : num_trials 
            f = all_fish(i).luminance(il).data(trial_idx);
  
            %% NEW: update validity to include both body and tail
            v = all_fish(i).luminance(il).data(trial_idx).valid_tail + all_fish(i).luminance(il).data(trial_idx).valid_body; 
            ...
            for rep = 1: 3
                ...
                % if v(rep) == 1
                if v(rep) == 2 %% v thereore equals 2
                    ...
                    raw(i).luminances(il).x_tail{end+1} = x_tail;
                    raw(i).luminances(il).y_tail{end+1} = y_tail;
...
%% 3. FFT computation and plots
close all
clc

for i = 1 % for the first fish only
    num_ils = numel(all_fish(i).luminance);
    colorbar_vec = nan(1, num_ils);
    colors = magma(num_ils + 1);
    legend_labels = cellfun(@(x) [num2str(x), ' lx'], num2cell(all_fish(i).lux_values), 'UniformOutput', false);
    fish_name = fishNames{i};

    data_struct_y_tail = {res(i).luminances.y_tail};
   
    ytail_data_comb = cell(1,length(data_struct_y_tail));
    for k = 1:length(data_struct_y_tail)
        n_trials = length(data_struct_y_tail{k});
        if n_trials >3
            colorbar_vec(k) = 1;
        end

        ytail_data_comb{k} = nan(n_trials,length(t));
        data_tmp = data_struct_y_tail{k};

        for j = 1:n_trials
            data_tmp_tmp = data_tmp{j}* p2m * 100;
            data_tmp_tmp = data_tmp_tmp - mean(data_tmp_tmp);
            ytail_data_comb{k}(j,:) = data_tmp_tmp;

            [P1_pos,f] = single_side_specta(data_tmp_tmp,1,Fs,0,[0 5]);
            ytail_fft_pos_comb{k}(j,:) = P1_pos;

            data_tmp_tmp_vel = [0;diff(data_tmp_tmp)]*Fs;
            [P1_vel,f] = single_side_specta(data_tmp_tmp_vel,1,Fs,0,[0 5]);

            ytail_fft_vel_comb{k}(j,:) = P1_vel;
        end
    end

    % Set up the color bars
    colorbar_ticks = linspace(0,1,sum(colorbar_vec ==1)+1);
    colorbar_ticks = colorbar_ticks(1:end-1)+ 0.5*mean(diff(colorbar_ticks));

    for k = 1:length(ytail_fft_pos_comb)

        if size(ytail_fft_pos_comb{k},1)>3 % Toggles between "all trials" vs. "good trials only" uncomment for supplementary figure
            fft_pos_mean_tmp = mean(ytail_fft_pos_comb{k});
            ...
            % Smooth(data, window_size = 3) first, then take out the first 2 datapoints
            fft_pos_mean_tmp = smooth(fft_pos_mean_tmp, smooth_window_size);
            plot(f(3:end), fft_pos_mean_tmp(3:end),'linestyle','-','LineWidth',2, 'Color',colors(k,:))   
            ...
            xlabel('Freq (Hz)')
            ylabel('DFT Position')
            title('Fig 2G','FontWeight','normal','FontSize',16)
            cbh = colorbar;
            colormap(colors(colorbar_vec==1,:))
            cbh.Ticks = colorbar_ticks;
            cbh.TickLabels = legend_labels(colorbar_vec==1);
            cbh.TickLength = 0;

            fft_vel_mean_tmp = mean(ytail_fft_vel_comb{k});
           
            if count == 23
                saveas(gcf, [out_path, '\fig02G_fish_0', num2str(i), '_fft_tail_amp.png';]);
                saveas(gcf, [pdf_path, '\fig02G_fish_0', num2str(i), '_fft_tail_amp.pdf';]);
            end
            ...
            % Take out the first 2 datapoints
            fft_vel_mean_tmp = smooth(fft_vel_mean_tmp, smooth_window_size);
            plot(f(3:end), fft_vel_mean_tmp(3:end),'linestyle','-','LineWidth',2, 'Color',colors(k,:))   
            ...
            xlabel('Freq (Hz)')
            ylabel('DFT Velocity')
            title('Fig 2H','FontWeight','normal','FontSize',16)
            cbh = colorbar;
            colormap(colors(colorbar_vec==1,:))
            cbh.Ticks = colorbar_ticks;
            cbh.TickLabels = legend_labels(colorbar_vec==1);
            ...
            if count == 24
                saveas(gcf, [out_path, '\fig02H_fish_0', num2str(i), '_fft_tail_vel.png';]);
                saveas(gcf, [pdf_path, '\fig02H_fish_0', num2str(i), '_fft_tail_vel.pdf';]);
            end
        end
    end
end
